    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
      ...
      <div class="sm:flex sm:justify-between sm:items-center mb-5">
        ...
        >
          ...
          <!-- Add member button -->
          <PopupModal message="New Journal Entry"> </PopupModal>
        </div>
      </div>
      ...
      <div class="bg-white shadow-lg rounded-sm border border-gray-200 mb-8">
        <header class="px-5 py-4">
          <h2 class="font-semibold text-gray-800">
            Journal Entries
            <span class="text-gray-400 font-medium">{{ entries.length }}</span>
          </h2>
        </header>
          ...
          <div class="overflow-x-auto">
            <table class="table-auto w-full">
              ...
              <tbody class="text-sm divide-y divide-gray-200">
                <!-- Row -->
                <tr v-for="entries in temp" :key="entries.id">
                  <td
                    class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px"
                  >
                    <div class="flex items-center">
                  ...
                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    <div class="font-medium text-green-500">
                      {{ entries.customer.name }}
                    </div>
                  </td>

                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    <div>{{ entries.discount }}</div>
                  </td>
                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                  ...
                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    <div
                      ...
                      :class="statusClass(entries.entries_status)"
                    >
                      {{ capitalizeString(entries.entries_status) }}
                    </div>
                  </td>
                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    <div>{{ entries.generated_by.email }}</div>
                  </td>
                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    <div>{{ generateDateFormat(entries.created_at) }}</div>
                  </td>
                  <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                    <div class="flex items-center">
                      <div>{{ generateDateFormat(entries.due_date) }}</div>
                    </div>
                  </td>
                  <td
                    class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px"
...
export default {
  ...
  beforeMount() {
    this.entries = this.$store.getters["accounting/getJournal"];
  },
